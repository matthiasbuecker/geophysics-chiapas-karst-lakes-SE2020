...
layered = '02_smooth_models/MET_Line1_tr1_rc-cs001_s0';
smooth = '02_smooth_models/MET_Line1_tr1_rc-cs001_s0';

%% Read in data
% Read sounding x positions
xz = dlmread('MET_distances_and_depths.txt','',1,1);
xz = flipud(xz(1:12,:));
x = xz(:,5);
% Read smooth models (for background)
for ss=1:length(sn)
    d(:,:,ss) = dlmread(['../' smooth '/met' num2str(sn(ss)) '.txt'],'',19,0);
end
% Layer thicknesses
...
for ss=1:length(sn)
    dl(:,:,ss) = dlmread(['../' layered '/met' num2str(sn(ss)) '.txt'],'',19,0);
end
% Layer thicknesses
...
X = linspace(min(xz(:,5))-xextra,max(xz(:,5))+xextra,100);
Y = linspace(0,80,50);
[XX,YY] = meshgrid(X,Y);
VV = F(XX/xs,YY);
...
set(gca,'FontSize',fs)
axis ij
h.DataAspectRatio = [1,1/1.5,1]; % axis equal
xlim([min(xz(:,5))-20 max(xz(:,5))+20])
ylim([0 80])
