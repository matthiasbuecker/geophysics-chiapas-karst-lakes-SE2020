...
zoffset = 17.2+2.8;       	% Depth offset (with respect to water surface)
xoffset = 25;               % X offset relative to longer SBP profile 8m)
...
elem=dlmread(['../03_models/grids/' namary num2str(ary) '.elm']); 
elec=dlmread(['../03_models/grids/' namary num2str(ary) '.elc']); 
if elem(4,1)==11
    headerrows=4; 
else
    headerrows=3;
...
% Set size of axes
axes('units','centimeters','Position',[1.2 0.5 12 4],'Box','on');
...
h = gca;
h.XAxis.MinorTickValues=0:10:2000;
h.YAxis.MinorTickValues=0:10:200;
xlabel('Distance (m)','fontsize',fs)
ylabel('Depth (m)','fontsize',fs)
set(gca,'FontSize',fs)
axis ij
axis([xoffset max(elec(:,2))+xoffset 0 120]) % axis tight
