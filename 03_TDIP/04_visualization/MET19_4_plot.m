...
xoffset = 440;               % X offset relative to longer SBP profile 8m)
cmin=-3;                    % Minimum coverage
inv=5;                      % Inversion number to be plotted
namary='MET'; ary=194; sep=5; nel=24;
...
% Set figure size
figure('units','centimeters','position',[20,10,7.5,4.5]);
% Set size of axes
axes('units','centimeters','Position',[1 1.3 6 4],'Box','on');
...
set(gca,'FontSize',fs)
axis ij
axis([xoffset max(elec(:,2))+xoffset 15 45]) % axis tight
h.DataAspectRatio = [1,1/1.2,1]; % axis equal
% Colorbar
caxis(rrange);
hcb=colorbar('fontsize',fs,'location','SouthOutside');
set(get(hcb,'xlabel'),'String','\rho (\Omegam)',...
    'fontsize',fs)
set(hcb,'XTickMode','manual')
set(hcb,'XTick',[0 0.5 1 1.5 2 2.5 3])
set(hcb,'XTickLabel',{'1','3','10','32','100','316','1000'})
set(hcb,'TickLength',0.015)
